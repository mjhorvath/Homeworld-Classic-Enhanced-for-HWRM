...
function DetermineAntiChassisDemand(enemyIndex)
	...
	if (DefinedRolesActivated == 1) then
		FiValue = PlayersMilitary_Fighter(player_enemy, player_total)
		self_AFiValue = PlayersMilitary_AntiFighter(player_ally, player_total)
	end
	...
	-- DEFINED ROLES stuff
	if (DefinedRolesActivated == 1) then
		CoValue = PlayersMilitary_Corvette(player_enemy, player_total)
		self_ACoValue = PlayersMilitary_AntiCorvette(player_ally, player_total)
	end
	...
	-- DEFINED ROLES stuff
	if (DefinedRolesActivated == 1) then
		FrValue = PlayersMilitary_Frigate(player_enemy, player_total)
		self_AFrValue = PlayersMilitary_AntiFrigate(player_ally, player_total)
	end
...
function DetermineChassisDemand(enemyIndex)
	...
	-- DEFINED ROLES stuff
	if (DefinedRolesActivated == 1) then
		AFiValue = PlayersMilitary_AntiFighter(player_enemy, player_total)
		ACoValue = PlayersMilitary_AntiCorvette(player_enemy, player_total)
		AFrValue = PlayersMilitary_AntiFrigate(player_enemy, player_total)
	end
...
function DetermineSpecialDemand()
	...
	-- DEFINED ROLES stuff
	if (DefinedRolesActivated == 1) then
		numInterceptors = PlayersUnitTypeCount(player_ally, player_total, eINTERCEPTOR)
	end
        if (FIGHTERPRODUCTION ~= nil) then
		numFighterProduction = NumSubSystems(FIGHTERPRODUCTION)
        end
		if (ShipDemandGet( kInterceptor ) > 0 and numFighterProduction > 0 and numInterceptors < 3 and s_militaryPop < 10) then
			ShipDemandAdd( kInterceptor, 0.5 )
		end
	else
		if (ShipDemandGet( kInterceptor ) > 0 and IsResearchDone(FIGHTERDRIVE) == 1 and numInterceptors < 7 and s_militaryPop < 22) then
			ShipDemandAdd( kInterceptor, 0.5 )
		end
	end
	...
	-- DEFINED ROLES stuff
	if (DefinedRolesActivated == 1) then
		numSpecial = numQueueOfClass(eCapture) + numQueueOfClass(eShield) + PlayersUnitTypeCount(player_ally, player_total, eCapture) + PlayersUnitTypeCount(player_ally, player_total, eShield)
		numFrigates = numQueueOfClass(eFrigate) + PlayersUnitTypeCount(player_ally, player_total, eFrigate)
	end
	...
	-- DEFINED ROLES stuff
	if (DefinedRolesActivated == 1) then
		numCollecting = PlayersUnitTypeCount(player_ally, player_total, eCollector)
	end
 	if ( (numCollecting > 9 or GetRU() > 1500) and s_militaryPop > 8 and UnderAttackThreat() < -75) then
...
function DetermineScoutDemand()	
	...
	if (UnderAttackThreat() < -10 and s_militaryPop > 0 and gameTime() > sg_randScoutStartBuildTime) then			
		...
		-- DEFINED ROLES stuff
		if (DefinedRolesActivated == 1) then
			shipCount = PlayersUnitTypeCount(player_ally, player_total, eScout)
		end
...
function DetermineBuilderClassDemand()
	...
	-- DEFINED ROLES stuff
	if (DefinedRolesActivated == 1) then
		numBuilders = numQueueOfClass( eBuilder )
		numActiveBuilders = numActiveOfClass( s_playerIndex, eBuilder ) + PlayersUnitTypeCount(player_ally, player_total, eBuilder)
	end
	...
	if (numCollecting < 7 and numRUs < 15000 ) then
		return
	end
