...
function CpuBuild_PersonalityDemand()
	...
	AI_trace("PersonalityDemand: Fi:"..sg_classPersonalityDemand[ eFighter ].." Co:"..sg_classPersonalityDemand[ eCorvette ].." Fr:"..sg_classPersonalityDemand[ eFrigate ])
end
...
function DetermineAntiChassisDemand(enemyIndex)
	...
	AI_trace("AChDmd: AFi:"..AFiDemand.." ACo:"..ACoDemand.." AFr:"..AFrDemand)
end
...
function DetermineSpecialDemand()
	if kInterceptor ~= nil then
		...
		if (SelfRace_GetNumber("cfg_buildable_subsystems", 1.0) >= 1.0) then
			...
			if (ShipVariantDemandGet( kInterceptor ) > 0 and numFighterProduction > 0 and NumVariantSquadrons(kInterceptor) < 3 and s_militaryPop < 10) then
				ShipVariantDemandAdd( kInterceptor, 0.5 )
			end
		-- kus and tai
		else
			local temp_fighterdrive = nil
			if KUS_FIGHTERDRIVE ~= nil then
				temp_fighterdrive = KUS_FIGHTERDRIVE
			elseif TAI_FIGHTERDRIVE ~= nil then
				temp_fighterdrive = TAI_FIGHTERDRIVE
			end
			if (ShipVariantDemandGet( kInterceptor ) > 0 and IsVariantResearchDone(temp_fighterdrive) == 1 and NumVariantSquadrons(kInterceptor) < 7 and s_militaryPop < 22) then
				ShipVariantDemandAdd( kInterceptor, 0.5 )
			end
		end
	end
	...
	if(numFrigates == 0 or numSpecial / numFrigates > 0.4) then
 	...
 	if ( (GetNumCollecting() > 9 or GetRU() > 1500) and s_militaryPop > 8 and UnderAttackThreat() < -75) then
		if kBattleCruiser ~= nil then ShipVariantDemandAdd( kBattleCruiser, 0.75 ) end
		if kDestroyer ~= nil then ShipVariantDemandAdd( kDestroyer, 0.25 ) end

		-- hgn and vgr
		if (SelfRace_GetNumber("cfg_bc_can_upgrade", 0.0) >= 1.0) then
			if (IsVariantResearchDone( "hgn_battlecruiserionweapons" ) == 1) then
				if kBattleCruiser ~= nil then ShipVariantDemandAdd( kBattleCruiser, 0.5 ) end
			end
		-- kus and tai
		else
			if kDestroyer ~= nil then ShipVariantDemandAdd( kDestroyer, 0.35 )  end
			if kMissileDestroyer ~= nil then ShipVariantDemandAdd( kMissileDestroyer, 0.65 ) end
			if KUS_HEAVYGUNS ~= nil then
				if (IsVariantResearchDone( "kus_heavyguns" ) == 1) then
					if kBattleCruiser ~= nil then ShipVariantDemandAdd( kBattleCruiser, 0.6 ) end
				end
			end
			if TAI_HEAVYGUNS ~= nil then
				if (IsVariantResearchDone( "tai_heavyguns" ) == 1) then
					if kBattleCruiser ~= nil then ShipVariantDemandAdd( kBattleCruiser, 0.6 ) end
				end
			end
		end		
...
function CpuBuild_DefaultShipDemandRules_Easy()	
	...
	AI_trace("Aim:"..minMilitaryValue.." CurMil:"..s_selfTotalValue.." Enm:"..s_enemyTotalValue )
...
function DoMilitaryBuild()
	...
	if (UnderAttackThreat() > -5 and shipId > 0 and highestPriority >= 0.5) then
		ssDemand = 0
		AI_trace("Subsys demand reduced because we are under attack")
	end	
