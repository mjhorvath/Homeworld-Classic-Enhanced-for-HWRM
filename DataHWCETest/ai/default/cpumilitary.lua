...
function Logic_military_attackrules()
	if (g_LOD == 0) then
		...
		if (s_selfTotalValue*2 < s_enemyTotalValue and s_selfTotalValue > 150) then
			cp_attackPercent = 0
			AI_trace("I'm Losing!! Go on defence")
		end
	end
end
	
function attack_now_timer()	
	AI_trace("Script:calling attack_now_timer")
	AttackNow();
...
function Logic_military_setattacktimer()  
	...
	local temp_cpuplayers_defensive = nil
	local temp_cpuplayers_aggressive = nil
	local temp_cpuplayers_dynamic = nil

	-- races are hardcoded here, needs fixing
	if HGN_CPUPLAYERS_DEFENSIVE ~= nil then
		temp_cpuplayers_defensive = HGN_CPUPLAYERS_DEFENSIVE
		temp_cpuplayers_aggressive = HGN_CPUPLAYERS_AGGRESSIVE
		temp_cpuplayers_dynamic = HGN_CPUPLAYERS_DYNAMIC
	elseif KUS_CPUPLAYERS_DEFENSIVE ~= nil then
		temp_cpuplayers_defensive = KUS_CPUPLAYERS_DEFENSIVE
		temp_cpuplayers_aggressive = KUS_CPUPLAYERS_AGGRESSIVE
		temp_cpuplayers_dynamic = KUS_CPUPLAYERS_DYNAMIC
	elseif TAI_CPUPLAYERS_DEFENSIVE ~= nil then
		temp_cpuplayers_defensive = TAI_CPUPLAYERS_DEFENSIVE
		temp_cpuplayers_aggressive = TAI_CPUPLAYERS_AGGRESSIVE
		temp_cpuplayers_dynamic = TAI_CPUPLAYERS_DYNAMIC
	elseif VGR_CPUPLAYERS_DEFENSIVE ~= nil then
		temp_cpuplayers_defensive = VGR_CPUPLAYERS_DEFENSIVE
		temp_cpuplayers_aggressive = VGR_CPUPLAYERS_AGGRESSIVE
		temp_cpuplayers_dynamic = VGR_CPUPLAYERS_DYNAMIC
	end

	-- "temp_cpuplayers_defensive" and its siblings have no variants so it's okay to use "IsResearchDone" here instead of "IsVariantResearchDone"
	-- however, to make the script future-proof, should I maybe check for variants anyway?
	if temp_cpuplayers_defensive ~= nil then
		cpuplayers_delay = 4			
		if IsResearchDone( temp_cpuplayers_defensive ) == 1 then
			-- do nothing
		elseif IsResearchDone( temp_cpuplayers_aggressive ) == 1 then
			cpuplayers_delay = 0.5			
		elseif IsResearchDone( temp_cpuplayers_dynamic ) == 1 then
	...
	if (gametime >= timedelay or HaveBeenAttacked()==1) then
		if (Rule_Exists("attack_now_timer") == 0) then
			AI_trace("Script: Attacktimer added")			
