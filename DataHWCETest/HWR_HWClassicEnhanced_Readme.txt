...
Authors:	Mikali, Luke Moloney (original version of HWC mod) and many, 
			many individual map authors (see map thumbnails for details)
Created:	2011/11/18
Updated:	2020/08/12
...
• Currently, enabling the RESEARCH OFF rule automatically turns the BENTUSI 
  ...
  is formed.
• The added production systems on "Vgr_Carrier_car1" are misaligned. 
...
• The map "HW1_ClubOMO8.level" crashes to desktop and I don't know why. There 
  are possibly other maps that do this as well.
• The fighter and corvette subsystems on the modified Vaygr carrier are turned 
  in the wrong direction.
• After collecting a crystal, it is drained of RUs and spit out the top of the 
  ship. This needs to be fixed.
• Vaygr carrier subsystem animation not working again.
• Is there a way to show an asteroid as a dot outside of Sensors Manager? Some 
  of them are really small and normally simply disappear.
...
2.4.0 --- 2020/08/12
• Renamed a lot of map objects so they wouldn't conflict with the default 
  objects.
• Removed text clouds from the mod and moved them into the GitHub repo instead.
• Removed some starting fleets so that only changes in the number of ships need 
  their own files.
• Renamed some logging variables.
• Renamed "HumanPlayer" restriction table to "PlayerHuman". Added a new table 
  called "PlayerComputer" for AI players.
• Added "Test Gallery Resources" and "Test Gallery Ships" levels. They should 
  be disabled by default however.
