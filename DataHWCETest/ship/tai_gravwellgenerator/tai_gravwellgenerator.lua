...
SourceGravityWellGeneratorShipTable =
{
...
GravityWellGeneratorShipTable = FillVariantsFromTable(SourceGravityWellGeneratorShipTable)

SourceGravityWellGeneratorSalvageTable = FillVariantsFromTable({"kus_salvagecorvette","tai_salvagecorvette"})

GravityWellGeneratorSalvageList = ConvertTableToList(SourceGravityWellGeneratorSalvageTable)
...
function Do_Tai_GravWellGenerator(CustomGroup,playerIndex,shipID)
  local listCount = getn(GravityWellGeneratorShipTable)
  local tumbleRange = 100

  for i = 0, Universe_PlayerCount()- 1 do
    if (Player_IsAlive(i)== 1) then
      for x = 1, listCount do
        SobGroup_FillShipsByType("GravWell_Temp0", "Player_Ships"..i, GravityWellGeneratorShipTable[x])
