...
function Update_Kus_Mothership(CustomGroup, playerIndex, shipID)		
	...
	-- I have not created any variants of this ship so the existing code is still okay - Mikali
	for i = 0,5,1 do
	...
	--SP
	-- a hardcoded player number is okay since this is for singleplayer only
	if Player_GetNumberOfSquadronsOfTypeAwakeOrSleeping(-1, "Special_Splitter" ) == 0 then		
		...
		--btn hyperspace
		-- why Player_Ships0 instead of a different group per each player index?
