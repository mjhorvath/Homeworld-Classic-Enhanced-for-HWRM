...
if not (RUScale) then	RUScale	 = 2	end
...
-- HWRM file name, RU amount, number of latch points, cloud scale
ResourceTable =
{
	-- hw1: 0 (11), 25, 40, 100, 400
	-- hwc: 0, 160, 720, 2600, 7920
	-- hw2: 0, 0, 9000, 18000, 40000
	asteroid0 = {"asteroid0",   0, 1, 0,},
	asteroid1 = {"asteroid1",  25, 1, 0,},
	asteroid2 = {"asteroid2",  40, 1, 0,},
	asteroid3 = {"asteroid3", 100, 2, 0,},
	asteroid4 = {"asteroid4", 400, 3, 0,},
	-- hw1: 50, 108, 250, 700, ???
	-- hwc: 50, 108, 250, 700, ???
	dustcloud0 = {"dustcloud0",   50, 1,  100,},
	dustcloud1 = {"dustcloud1",  108, 1,  200,},
	dustcloud2 = {"dustcloud2",  250, 1,  600,},
	dustcloud3 = {"dustcloud3",  700, 1, 1200,},
	dustcloud4 = {"dustcloud3", 1500, 1, 1800,},
	-- hw1: 100, 100, 100, 100
	-- hwc: 100, 100, 100, 100
	nebula0 = {"nebula0", 100, 1, 85.75,},
	nebula1 = {"nebula0", 100, 1, 85.75,},
	nebula2 = {"nebula0", 100, 1, 85.75,},
	nebula3 = {"nebula0", 100, 1, 85.75,},
	nebula4 = {"nebula0", 100, 1, 85.75,},
	-- hwc: 0, 0
	-- hwc: 6000, 10000
	dcrystal0 = {"hwccrystal_1",  6000, 1, 0,},
	dcrystal1 = {"hwccrystal_2", 10000, 1, 0,},
}
...
--			else
--				print("HWCM: Formation type " .. sFormation .. " not recognized.")
--			end
...
--				AddSquadrons(SquadName .. "_" .. j, ShipType, ShipPosition, iPlayer, ShipRotation, iNumber, 0, sRace)
			...
			print("HWCM: Extra player squadrons found in non-player fleet.")
		end
	end
end
...
--			end
			...
			print("HWCM: Extra out-of-range squadrons found in player fleet.")
		end
	end
end
...
--	iNumber = 10
	...
	if ((RUType ~= "") and (RUType ~= nil) and (iPlayer < maxPlayers)) then
		...
		print("HWCM: AddSquadrons: ship type " .. sType .. " not recognized. Skipping.")
	end
end
...
function AddResources(tPosition, sLayout, sDistribution, iNumResources, fA, fB, fRotY, fRotZ)
	...
	for k, iCount in Distributions[sDistribution] do
		if (ResourceTable[k]) then
			...
			print("HWCM: AddResources: resource type " .. k .. " not recognized.")
		end
	end
	if (Relative > 0) then
		for k, iCount in Distributions[sDistribution] do
			if (iCount > 0) then
				...
				for j = 1, Amount do
					...
					elseif (sLayout == "nebula") then
						print("HWCM: AddResources: A nebula got into my resources! Bad nebula!!")
					end
--					print_objects("-- sLayout = " .. sLayout .. ", cooX = " .. cooX .. ", cooY = " .. cooY .. ", cooZ = " .. cooZ .. ", j = " .. j)
					...
					if ((PebRatio > 0) and (CountPebbles < PebNumo)) then
						...
						if ((k == "dustcloud0") or (k == "dustcloud1") or (k == "dustcloud2") or (k == "dustcloud3") or (k == "dustcloud4")) then		-- dustcloud4 doesn't exist I think
...
--						print_objects("HWCM: AddResources: added RUType = " .. RUType .. " at " .. vstr(tCoordinates))
					end
...
function convert_position(tPosition)
	return vround({tPosition[1] * MapScale * -1, tPosition[3] * MapScale, tPosition[2] * MapScale})
end
