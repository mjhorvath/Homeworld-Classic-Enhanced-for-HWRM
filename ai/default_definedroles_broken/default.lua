...
function CountAllies()
	local count = 0
	for i = 0, (PlayersTotal() - 1) do
		if (Player_IsEnemy(i) == 0) then
			count = count + 1
		end
	end
	return count
...
function CacheCurrentState()
	...
	if (DefinedRolesActivated == 1) then
		s_numAlliesToCount = CountAllies()
		s_numEnemiesToCount = PlayersTotal() - s_numAlliesToCount
		s_totalProdSS = ((s_numFiSystems + s_numCoSystems) + s_numFrSystems)
		s_militaryPop = PlayersMilitaryPopulation(player_ally, player_total) / s_numAlliesToCount
		s_selfTotalValue = PlayersMilitary_Total(player_ally, player_total) / s_numAlliesToCount
		s_enemyTotalValue = PlayersMilitary_Total(player_enemy, player_total) / s_numEnemiesToCount
--		s_militaryPop = PlayersMilitaryPopulation(player_ally, player_total)
--		s_selfTotalValue = PlayersMilitary_Total(player_ally, player_total)
--		s_enemyTotalValue = PlayersMilitary_Total(player_enemy, player_max)
		...
		-- from cpuresource.lua
		sg_minNumCollectors = 5 * s_numAlliesToCount
		sg_maxNumCollectors = 30 * s_numAlliesToCount
	else
		s_totalProdSS = s_numFiSystems + s_numCoSystems + s_numFrSystems
